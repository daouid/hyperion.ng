...
bool LedDeviceLifx::init(const QJsonObject &deviceConfig)
{
	...
	if ( isInitOK )
	{
		...
		_multiCastGroupPort = static_cast<quint16>(deviceConfig["port"].toInt(56700));
...
bool LedDeviceLifx::initNetwork()
{
	bool isInitOK = true;
...
void LedDeviceLifx::setColor(int orbId, const ColorRgb &color, int commandType)
{
	QByteArray bytes;
...
void LedDeviceLifx::sendCommand(const QByteArray &bytes)
{
