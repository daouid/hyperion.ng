...
bool LedDeviceLifx::init(const QJsonObject &deviceConfig)
{
	...
	if ( isInitOK )
	{
		...
		{
			//Discover Lifx device
			if ( !discoverDevice() )
			{
				this->setInError("No target IP defined nor Lifx device was discovered");
				return false;
			}
		}
...
bool LedDeviceLifx::initLeds()
{
	...
	QString url = getUrl(_hostname, _api_port, API_ROOT );
	...
	else
	{
		...
		else
		{
			...
			{
				Warning(_log, "Lifx: More panels [%u] than configured LEDs [%u].", _panelLedCount, configuredLedCount );
			}
		}
	}
	return isInitOK;
...
bool LedDeviceLifx::discoverDevice()
{
	...
	if ( address.isEmpty() ) {
		...
		if ( address.isEmpty() ) {
			Warning(_log, "No Lifx device discovered");
		}
	}

	// Canvas or Light Panels found
	if ( ! address.isEmpty() ) {
		Info(_log, "Lifx device discovered at [%s]", QSTRING_CSTR( address ));
		isDeviceFound = true;
...
QJsonDocument LedDeviceLifx::changeToExternalControlMode()
{

	QString url = getUrl(_hostname, _api_port, API_EFFECT );
...
QString LedDeviceLifx::getUrl(QString host, QString port, QString endpoint) const {
	return QString(API_URL_FORMAT).arg(host, port, endpoint);
}

QJsonDocument LedDeviceLifx::getJson(QString url)
{
...
QJsonDocument LedDeviceLifx::putJson(QString url, QString json)
{
...
QJsonDocument LedDeviceLifx::handleReply(QNetworkReply* const &reply )
{
...
QString LedDeviceLifx::getOnOffRequest (bool isOn ) const
{
	QString state = isOn ? STATE_VALUE_TRUE : STATE_VALUE_FALSE;
	return QString( "{\"%1\":{\"%2\":%3}}" ).arg(STATE_ON, STATE_ONOFF_VALUE, state);
}

int LedDeviceLifx::switchOn()
{
	if ( _deviceReady)
	{
		// Set Lifx to External Control (UDP) mode
		Debug(_log, "Set Lifx to External Control (UDP) streaming mode");
		...
		//Switch on Lifx device
		QString url = getUrl(_hostname, _api_port, API_STATE );
		putJson(url, this->getOnOffRequest(true) );
	}
	return 0;
}

int LedDeviceLifx::switchOff()
{
...
std::string LedDeviceLifx:: uint8_vector_to_hex_string( const std::vector<uint8_t>& buffer ) const
{
	std::stringstream ss;
